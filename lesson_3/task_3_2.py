...
def function_add_string(list_new,number_of_digits):
    list_add = ' '                           # задає стрічці, у яку склеюватимемо цифри початкове значення
    for i in range(number_of_digits):        # 
       list_new[i] = str(list_new[i])        # конвертує кожен символ у цифру
       list_add += list_new[i]               # повертає основній програмі значення   
    return list_add

# ----- MAIN ----- #  
number_of_digits = 4                         # задає скільки цифр має мати число
...
# ----- 2.1 ----- #
product_of_numbers = 1                       #  ініціалізує змінну і присвоює початкове значення
